...
class DataSource:
    ...
    def getData(self, setname, dataType, fromDate, toDate=20191009):
        ...
        try:
            ...
            query = "SELECT pricedate, %s FROM {0} WHERE pricedate BETWEEN to_date(%s::text, 'YYYYMMDD') AND to_date(%s::text, 'YYYYMMDD')"
            cursor.execute(query, (quote_ident(setname, cursor), dataType, fromDate, toDate, ))
